...
def execute_selected_tasks():
    """Execute all selected tasks in batch"""
    try:
        # Load current tasks
        tasks_data = load_json_file('phased_tasks.json')
        ...
        selected_tasks = [task for task in tasks_data if task.get('selectionStatus') == 'selected']
        
        if not selected_tasks:
            return jsonify({'error': 'No tasks selected for execution'}), 400
        ...
        try:
            ...
            if result.returncode == 0 or files_created:
                # Mark selected tasks as executed
                for task in tasks_data:
                ...
                success_message = f'Successfully executed {len(selected_tasks)} selected tasks'
...
def get_stats():
    ...
    try:
        ...
        stats = {
            ...
            'selectedTasks': len([t for t in tasks if t.get('selectionStatus') == 'selected']),
            'pendingSelection': len([t for t in tasks if t.get('selectionStatus') == 'pending']),
            'executedTasks': len([t for t in tasks if t.get('selectionStatus') == 'executed']),
